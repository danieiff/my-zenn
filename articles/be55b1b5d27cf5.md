---
title: "業務アプリに向けBaaS vte.cx (by Virtual Technology（株）"
emoji: "️🖥"
...
vte.cx(ヴィーテックス) [開発者による入門記事](https://qiita.com/stakezaki/items/e526ca061d8f004db7f5) と 当時の雰囲気↓
...
手順通りにAPIサーバーを立てて認証を済ませたら、**エンドポイント**と**スキーマ**だけ設定します。
すると、ユーザーに好き放題入力させて、POSTリクエストに乗せてサーバーに送ることができます。
```ts
axios.post('/d/{エンドポイント}', {/* スキーマ定義通りの内容のオブジェクト */})
```
:::details フォームコンポーネントの説明
```ts:Register.tsx
import * as React from 'react' // vte.cxでは開発時ビルドを早くするために、reactモジュールをこのようにimportします。
