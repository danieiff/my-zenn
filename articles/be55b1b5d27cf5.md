...
vte.cx(ブイテックス) [歩き方](https://qiita.com/stakezaki/items/e526ca061d8f004db7f5) と 当時の雰囲気↓
...
const getFeed = async () => {
 ...
 // フィードを取得する｡
  fallback(()=>{
    const feed = await axios.get(`/d/users?${query}&n=${page}&l=${LENGTH}`)
...
```
以下のクエリパラメータに検索用パラメータを追加します。
カーソルを作るクエリパラメータ
 `/d/{エンドポイント}?{検索用パラメータ}&_pagination=${cursorEnd.current},${ page + RANGE - 1 }&l=${LENGTH}`
フィードを取得するクエリパラメータ
```diff ts:Table.tsx
...
+ const history = useHistory
  const feed = await axios.get(`/d/users?${query}&n=${page}&l=${LENGTH}`)
+ history.push('/d/users?{検索用パラメータ}&n={page}&l=${LENGTH}')
  setState({ feed: SEIKEI(feed.data) }) // 整形する (2)参照
```
## 総括
CSSを書かず､Material UIのコンポーネントをそのまま使いまくりました。
